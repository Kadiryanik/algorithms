...
unsigned long long karatsuba(unsigned long long x, unsigned long long y){
	...
	if((xLength + yLength) < 5){ // TODO: best value?
		return x*y;
	}

	Longer = (Longer / 2) + (Longer % 2);
	long long bPowerM = pow(10, Longer);

	//19 x 10^2 + 63 ----- 20 x 10^2 + 16
	//x0          x1       y0          y1
	long long x0 = x / bPowerM;
	long long x1 = x - (x0 * bPowerM);
	long long y0 = y / bPowerM;
	long long y1 = y - (y0 * bPowerM);

	long long z0 = karatsuba(x1, y1);
	long long z2 = karatsuba(x0, y0);
	long long z1 = karatsuba(x1 + x0, y1 + y0) - (z0 + z2);

	long long P = z0 + (z1 * bPowerM) + (z2 * (long long)(pow(10, 2 * Longer)));
